...
bool xmrig::BlockTemplate::parse(bool hashes)
{
    ...
    // Skip additional coinbase outputs (PBC has dev fund + vesting outputs)
    for (uint64_t k = 1; k < m_numOutputs; ++k) {
        uint64_t extra_amount;
        ar(extra_amount);
        uint8_t extra_output_type;
        ar(extra_output_type);
        Span extra_key;
        ar(extra_key, kKeySize);
        if (extra_output_type == 3) {
            uint8_t extra_view_tag;
            ar(extra_view_tag);
        }
    }
    ...
    while (ar_extra.index() < m_extraSize) {
        ...
        case 0x00: // TX_EXTRA_TAG_PADDING
            // padding can be 0-255 zero bytes, but in varint encoding just skip 0
            break;

        default:
            // PBC and other CryptoNote forks may have custom tags in coinbase extra.
            // Read the size varint and skip the payload instead of failing.
            ar_extra(size);
            ar_extra.skip(size);
            break;
        }
