...
const char *Algorithm::kRX_PBC          = "rx/pbc";
...
#   ifdef XMRIG_ALGO_RANDOMX
    ...
    ALGO_ALIAS_AUTO(RX_PBC),        ALGO_ALIAS(RX_PBC,          "randomx/pbc"),
                                    ALGO_ALIAS(RX_PBC,          "randompbc"),
...
std::vector<xmrig::Algorithm> xmrig::Algorithm::all(const std::function<bool(const Algorithm &algo)> &filter)
{
    static const std::vector<Id> order = {
        ...
        CN_UPX2,
        RX_0, RX_V2, RX_WOW, RX_PBC, RX_ARQ, RX_GRAFT, RX_SFX, RX_YADA,
